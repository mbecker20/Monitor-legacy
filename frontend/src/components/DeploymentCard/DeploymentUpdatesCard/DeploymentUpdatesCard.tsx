...
import './DeploymentUpdatesCard.css'
...
function DeploymentUpdatesCard({ updates }: { updates: Updates }) {
  return (
    <div className="InnerCard" style={{ gridArea: "updates" }}>
      ...
      <div className="FlexCol" style={{height: '20rem'}}>
        <div className='UpdatesBody'>
          {Object.keys(updates).map((key, i) => <DeploymentUpdate update={updates[key]} key={i}/>)}
        </div>
      </div>
    </div>
  );
...
function DeploymentUpdate({ update }: { update: Update }) {
...
export default DeploymentUpdatesCard;

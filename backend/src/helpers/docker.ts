...
function envString(environment?: EnvironmentVar[]) {
	return environment && environment.length > 0 ? environment.map(({ variable, value }) => ` -e "${variable}=${value}"`).reduce((prev, curr) => prev + curr) : ""
}
...
export async function createDockerRun({ buildID, image, latest, ports, environment, network, volumes, restart, postImage, containerName, containerUser, logToAWS, folderName }: Deployment, server?: Server) {
	...
	return `docker pull ${_image}${buildID || latest ? ":latest" : ""} && ` + `docker run -d --name ${containerName}` + containerUserString(containerUser) + portsString(ports) + volsString(folderName, volumes, server) + envString(environment) + restartString(restart) + networkString(network) + ` ${_image}${buildID || latest ? ":latest" : ""}${postImage ? " " + postImage : ""}`
}
...
export async function getAllContainerStatus(deploymentsAr: Deployment[]) {
	...
	return {
		...
		...Object.assign({}, ...(await Promise.all(servers.filter((server: any) => server.name !== SYSTEM_SERVER_NAME).map(async (server: any) => await allContainerStatusRemote(deploymentsAr, server))))),
	}
}
